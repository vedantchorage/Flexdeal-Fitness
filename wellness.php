...
// Query for Welness_BCA products
$welnessBCAQuery = "SELECT * FROM products WHERE LOWER(type) = 'Welness' and Categories ='BCA'";
$welnessBCAResult = $pdo->query($welnessBCAQuery);
$welnessBCAProduct = $welnessBCAResult->fetchAll(PDO::FETCH_ASSOC);

// Query for Welness_Aqua products
// $aquaQuery = "SELECT * FROM products WHERE LOWER(type) = 'Welness'";
// $aquaResult = $pdo->query($aquaQuery);
// $aquaProduct = $aquaResult->fetchAll(PDO::FETCH_ASSOC);

// Query for Welness_Rehab products
$rehabQuery = "SELECT * FROM products WHERE LOWER(type) = 'Welness' and Categories = 'REHAB'";
$rehabResult = $pdo->query($rehabQuery);
$rehabProduct = $rehabResult->fetchAll(PDO::FETCH_ASSOC);

// Query for Welness_Pilates products
$pilatesQuery = "SELECT * FROM products WHERE LOWER(type) = 'Welness' and Categories = 'PILATES'";
$pilatesResult = $pdo->query($pilatesQuery);
$pilatesProduct = $pilatesResult->fetchAll(PDO::FETCH_ASSOC);

// Query for Welness_Pilates products
$aquaQuery = "SELECT * FROM products WHERE LOWER(type) = 'Welness' and Categories = 'Aqua'";
$aquaResult = $pdo->query($aquaQuery);
$aquaProduct = $aquaResult->fetchAll(PDO::FETCH_ASSOC);
...
/* Product Section Styles */
    ...
    /* Tablet adjustments */
    /* Product Card */
   /* Tablet adjustments */    /* Animation Delay for Each Card */
...
<body>
        ...
        <div class="container header-container">
            ...
            <nav>
                <ul id="main-nav">
                    ...
                    <li><a href="wellness.php" class="active">Wellness </i></a></li>
    ...
    <section class="product-section">
        ...
        <div class="products-container">
            ...
            <?php foreach ($welnessBCAProduct as $product): ?>
            <?php
    ...
    <div class="products-container">
        <?php foreach ($rehabProduct as $product): ?>
            <?php
                $imageArray = json_decode($product['image_paths'] ?? '', true);
                $imageArray = is_array($imageArray) ? $imageArray : [];
                $imageUrl = $product['image_url'] ?? '';
            ...
            <div class="product-card slide">
                <div class="product-image" data-images="<?= $allImagesJson ?>"
                    ...
                    data-assembly="<?= htmlspecialchars($product['Assemblyarea'], ENT_QUOTES) ?>"
                    onclick="handleQuickView(this)">
                    <img src="<?= htmlspecialchars($mainImage, ENT_QUOTES) ?>" alt="<?= htmlspecialchars($product['product_name'], ENT_QUOTES) ?>">
                    <div class="quick-view">Quick View</div>
                </div>
                <div class="product-info">
                    <span class="product-category">Wellness</span>
                    ...
                    <div class="product-actions">
                        <a href="#" class="get-quote-btn"
                            onclick="openQuoteForm('<?= htmlspecialchars($product['product_name']) ?>',
                            '₹<?= number_format($product['price']) ?>',
                            '<?= $allImagesJson ?>')">Get Quote</a>
                        <div class="action-icons">
                            <a href="https://wa.me/917776090461?text=*Enquiry%20for%20<?= urlencode($product['product_name']) ?>*%0A%0A
*Product%20Code:*%20<?= urlencode($product['product_code']) ?>%0A
*Price:*%20₹<?= urlencode(number_format($product['price'])) ?>%0A%0A
    ...
    <section class="product-section">
        <div class="section-header">
          ...
          <div class="products-container">
            ...
            <?php foreach ($pilatesProduct as $product): ?>
            <?php
                $imageArray = json_decode($product['image_paths'] ?? '', true);
                $imageArray = is_array($imageArray) ? $imageArray : [];
    ...
    <section class="product-section">
        <div class="section-header">
          ...
          <div class="products-container">
            ...
            <?php foreach ($aquaProduct as $product): ?>
            <?php
                $imageArray = json_decode($product['image_paths'] ?? '', true);
                $imageArray = is_array($imageArray) ? $imageArray : [];
